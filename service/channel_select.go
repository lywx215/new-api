...
func selectChannelWithRPMCapacity(c *gin.Context, group, modelName string, retry int, filters []dto.ChannelFilter) (*model.Channel, error) {
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			channel, err = selectChannelWithRPMCapacity(param.Ctx, autoGroup, param.ModelName, priorityRetry, filters)
			if err != nil {
		...
		channel, err = selectChannelWithRPMCapacity(param.Ctx, param.TokenGroup, param.ModelName, param.GetRetry(), filters)
		if err != nil {
...
func SelectChannelForRequest(c *gin.Context, modelName string, retry *RetryParam) (*model.Channel, string, *ChannelSelectError) {
	...
	if pin, found, overridden := constraints.ResolvedPin(); found {
		...
		if allowed, wait, _ := TryReserveOpenCodeGoRPM(c, channel); !allowed {
			return nil, "", &ChannelSelectError{StatusCode: http.StatusTooManyRequests, Code: types.ErrorCodeOpenCodeGoRPMLimit, Message: (&OpenCodeGoRPMError{RetryAfter: wait}).Error(), RetryAfter: wait}
		}
		return channel, "", nil
	}
	...
	if retry.GetRetry() == 0 {
		if preferredChannelID, found := GetPreferredChannelByAffinity(c, modelName, usingGroup); found {
			...
			if affinityUsable {
				if allowed, _, remaining := TryReserveOpenCodeGoRPM(c, channel); !allowed {
					MarkChannelAffinityMigration(c, channel.Id, 0, "rpm_soft_limit", remaining)
					channel = nil
					affinityUsable = false
					affinityRateLimited = true
				}
			}
			if !affinityUsable && !affinityRateLimited && !ShouldKeepChannelAffinityOnChannelDisabled() {
	...
	if channel == nil {
		...
		if err != nil {
			if rpmErr, ok := AsOpenCodeGoRPMError(err); ok {
				return nil, selectGroup, &ChannelSelectError{StatusCode: http.StatusTooManyRequests, Code: types.ErrorCodeOpenCodeGoRPMLimit, Message: rpmErr.Error(), RetryAfter: rpmErr.RetryAfter}
			}
