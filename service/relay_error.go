...
func ProcessChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) {
	...
	shouldDisable, reason := ShouldDisableChannel(channelError.ChannelType, err)
	if !channelError.AutoBan {
		shouldDisable = false
		reason = AutoDisableReasonChannelAutoBanDisabled
	}
	logger.LogError(c, fmt.Sprintf("channel error (channel #%d, channel type: %d, status code: %d, upstream status code: %d, auto-disable: %t, auto-disable reason: %s): %s", channelError.ChannelId, channelError.ChannelType, err.StatusCode, err.GetOriginalHTTPStatusCode(), shouldDisable, reason, common.LocalLogPreview(err.MaskSensitiveError())))
