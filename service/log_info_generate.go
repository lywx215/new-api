...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.PriceData.BillingSource != "" {
		other.SetPublic("pricing_source", relayInfo.PriceData.BillingSource)
		other.SetPublic("pricing_channel_type", relayInfo.PriceData.BillingChannelType)
	}
...
func InjectTieredBillingInfo(other *model.LogOther, relayInfo *relaycommon.RelayInfo, result *billingexpr.TieredResult) {
	...
	other.SetPublic("expr_hash", fmt.Sprintf("%x", common.Sha256Raw([]byte(snap.ExprString))))
	other.SetPublic("billing_source", snap.BillingSource)
	other.SetPublic("billing_channel_type", snap.ChannelType)
	if snap.BillingSource == "official" {
		other.SetPublic("official_billing_revision", billing_setting.OpenCodeGoOfficialBillingRevision)
	}
	if result != nil {
