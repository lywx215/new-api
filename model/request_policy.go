...
func BuildRequestPolicy(options map[string]string) (*RequestPolicySnapshot, error) {
	...
	for key, value := range raw {
		if field, ok := strings.CutPrefix(key, "channel_affinity_setting."); ok {
			switch field {
			case "enabled", "session_mode", "switch_on_success", "keep_on_channel_disabled", "max_entries", "default_ttl_seconds", "rules", "accept_internal_key", "generate_internal_key", "use_prompt_cache_key", "use_opencode_session", "use_metadata_user_id", "generate_fallback_key", "max_source_bytes", "affinity_ttl_seconds", "rpm_guard_enabled", "default_account_rpm", "account_burst", "rate_limit_cooldown_seconds":
			default:
